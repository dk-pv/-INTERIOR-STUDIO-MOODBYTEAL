...
function HeroDarkPanel({ inView }: { inView: boolean }) {
  ...
  return (
    <>
      <style>{`
        /* Dark panel — desktop: absolute right; mobile: stacks below heading */
        .hero-dark-panel {
      ...
      >
        ...
        <div style={{ marginBottom: 28 }}>
          {[
            ...
            { text: "Build", style: {} },
            { text: null, isCycling: true },
            { text: "Interiors.", style: { color: "rgba(245,244,240,0.22)", fontStyle: "italic" } },
          ].map((line, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.8, ease: EXPO, delay: 0.65 + i * 0.1 }}
            ...
            >
              ...
              >
                {line.isCycling ? <CyclingWord /> : line.text}
              </span>
            </motion.div>
          ))}
        </div>
...
export default function ContactPage() {
  ...
  // ── Faster slider: 1800ms, no blank flash ──────────────
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImage((prev) => (prev + 1) % contactImages.length);
    }, 1800);
  ...
  return (
    <>
      <style>{`
        ...
        /* On mobile the panel stacks, so justify from top with padding-top */
        @media (max-width: 768px) {
          .contact-hero {
            min-height: auto;
        ...
        @media (max-width: 768px) {
          ...
          /* Hide image panel on mobile — form takes full width */
          .contact-split-image {
            display: none;
          }
        }
      ...
      >
        ...
        <section data-theme="light" className="contact-split">
          {/* LEFT — Image slider (no blank flash: mode="sync" + absolute stack) */}
          <div
            ...
            {contactImages.map((src, i) => (
              <motion.div
                ...
                style={{
                  position: "absolute",
                  inset: 0,
                  // stack all images, z-index by active
