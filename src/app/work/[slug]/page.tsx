...
//   // Gallery fallback — use project.images if array exists, else just the cover
...
//     { label: "Location", value: (project as any).location ?? "UAE / India" },
//     { label: "Year", value: (project as any).year ?? "2024" },
//   ];
...
export default function ProjectDetailPage({
  ...
  // Gallery fallback
  const galleryImages: string[] = Array.isArray(project.images) && project.images.length > 0
    ? project.images
    : [project.image];
    ...
    <>
      ...
      <main style={{ backgroundColor: "#f9f8f6", color: "#0a0a0a" }}>
        ...
        <section
          style={{
            padding:
              "clamp(44px, 7vw, 72px) clamp(24px, 8vw, 120px) 0",
          }}
        >
          {/* Category label */}
          <motion.p
        ...
        <section
          style={{
            padding:
              "clamp(44px, 6vw, 64px) clamp(24px, 8vw, 120px)",
            display: "grid",
            gridTemplateColumns: "1fr clamp(240px, 28%, 360px)",
            gap: "clamp(40px, 8vw, 100px)",
            alignItems: "start",
        ...
        >
          ...
          >
            ...
            />
            {meta.map((m, i) => (
              <MetaPill key={m.label} label={m.label} value={m.value} delay={1.0 + i * 0.08} />
            ))}
          </motion.div>
        </section>
        ...
        <section
          style={{
            padding:
              "0 clamp(24px, 8vw, 120px) clamp(52px, 8vw, 80px)",
          }}
        >
          <SectionHeading text="Project Gallery" />
          <GalleryGrid images={galleryImages} />
        ...
        {/* ── PULL QUOTE ───────────────────────────────────────────────── */}
        <section style={{ padding: "0 clamp(24px, 8vw, 120px) clamp(52px, 8vw, 80px)" }}>
          <PullQuote
            text={
              project.quote ??
              "Design is not just what it looks like and feels like. Design is how it works within life, and how life works within it."
