  ...
  return (
    <>
      <style>{`
        ...
        /*
         * ── Mobile: hide corner labels completely ─────────────
         * Using visibility + pointer-events instead of display:none
         * so Framer Motion inline opacity/transform can't fight it.
         */
        @media (max-width: 640px) {
          .hero-corner-label {
      ...
      <section data-theme="dark" className="hero-section">
        ...
        {/* ── Corner labels — desktop/tablet only ── */}
        {CORNER_LABELS.map((item, i) => (
          <motion.div
            key={item.text}
            data-pos={item.pos}
        ...
        {/* ── Labels pill strip — mobile only (≤ 640px) ── */}
        <motion.div
          className="hero-labels-strip"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
