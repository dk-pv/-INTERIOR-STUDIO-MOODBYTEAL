...
// ─── NAV LINK ─────────────────────────────────────────
function NavLink({ href, children, active, isLight }: {
  href: string;
...
function CtaButton({ isLight }: { isLight: boolean }) {
  ...
  const borderColor  = isLight ? "rgba(10,10,10,0.22)" : "rgba(245,244,240,0.22)";
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      frameRef.current = requestAnimationFrame(() => {
        ...
        setIsLight((prev) => (theme === "light") !== prev ? theme === "light" : prev);
      });
    };
